...
class BalloonTreeController{
  ...
  List<Node> relationToNodes(List<Relation> relations, List<Concept> concepts){
    ...
    three.add(Node(relations[0].to_concept_id, childs, -1, concepts[concepts.indexWhere((element) => element.id == relations[0].to_concept_id)].concept));
    relations.forEach((element) {
      ...
      three.add(Node(element.concept_id, childs, parent, concepts[concepts.indexWhere((a) => a.id == element.concept_id)].concept));
    });
  ...
  findRelInNode(Concept element, List<Concept> concepts){
    Concept cons_parent = concepts[concepts.indexWhere((a) => a.id == element.aspectOf)];
    if(three.indexWhere((a) => a.id == cons_parent.aspectOf) == -1){
      findRelInNode(cons_parent, concepts);
    }


    three.add(Node(cons_parent.id, [], cons_parent.aspectOf, cons_parent.concept));
